...
################################################################################
#
#	Description: Quality check pipeline for Affymetrix Human Genome U95Av2 Array expression array. The script writes in the project workspace a file listing outliers identified with ArrayOutliers function
#
#	Command line use example: R --file=./QC_Affy_U95Av2.R --args "/scratch/jack/data/PhD/raw/Affy_U95Av2/BI-GDAC/target_BI-GDAC.txt" "/scratch/jack/data/PhD/Transcriptomics_project"
#
...
cat("Affymetrix GeneChip Human Genome U95Av2 Array data quality control.\n")
...
if (file.exists(paste(ProjectDir,"GenExpression_InputFiles.txt",sep="/"))){
    ...
    if ( studyID %in% Dataset2add[,"DatasetName"] ) {
        ...
        Dataset2add <- rbind(Dataset2add, matrix(c(studyID,paste(fileInfo,collapse="/"),"raw","Affy_U95Av2"),1,4,dimnames=list(studyID,colnames(Dataset2add))) )
    ...
    Dataset2add <- matrix(c(studyID,paste(fileInfo,collapse="/"),"raw","Affy_U95Av2"),1,4,dimnames=list(studyID,c("DatasetName","TargetFile","Type","Platform")))
	write.table( Dataset2add, paste(ProjectDir,"GenExpression_InputFiles.txt",sep="/"), row.names = FALSE, col.names = colnames(Dataset2add),  quote = FALSE, sep="\t" )
}
