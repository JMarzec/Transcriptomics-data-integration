################################################################################
#
#   File name: GenExpressionComb_Affy_U133A.R
#
...
################################################################################
#
#	Description: Pipeline combining datasets produced using Affymetrix Human Genome U133A Array. The script reads in three a priori generated files: (1) list of datasets to be analysed; (2) file produced with QC_Affy_U133A.R which lists outliers identified with ArrayOutliers function and (3) '.flat' file produced with ArrayAnnot.pl which lists reliable probes annotated with Ensembl gene IDs and gene symbol. Pipeline executes 'multiGene2ProbeFilter.R' to remove multi gene to probe mappings
#
#	Command line use example: R --file=./GenExpressionComb_Affy_U133A.R --args "/scratch/jack/data/PhD/Transcriptomics_project"
#
...
ProjectDir = args[4]
probesFlatFile = paste(paste(ProjectDir, collapse = "/"),"Affy_U133A.flat",sep="/")
...
DatasetInput=DatasetInput[DatasetInput[,"Platform"] %in% "Affy_U133A",]
...
cat("Loading datasets produced using Affymetrix GeneChip Human Genome U133A Array into an AffyBatch object...\n")
...
Affy_U133A_probes <- read.table(probesFlatFile,sep="\t",as.is=TRUE,header=TRUE,row.names=1)
...
difference=setdiff(ls(hgu133acdf),rownames(Affy_U133A_probes))
##### Remove these genes from the cdf environment -> the cdf will have only the interesting genes
rm(list=difference, envir=hgu133acdf)
...
Affy_U133A_gcrma <- NULL
Affy_U133A_gcrma <- gcrma(dat)

#save.image("Affy_U133A_gcrma.R")
#load("Affy_U133A_gcrma.R")

Affy_U133A_data <- exprs(Affy_U133A_gcrma)
...
Affy_U133A_datasets <- targetFile$Dataset

Affy_U133A_datasets_No <- max(as.numeric(factor(Affy_U133A_datasets)))

Affy_U133A_datasets.colour <- getDatasetsColours(Affy_U133A_datasets)


pdf("Affy_U133A_boxplot.pdf", pointsize = 8 ,width = 0.2*length(targetFile$FileName), height = 6)
par(mar=c(13, 4, 3, 2))
boxplot(dat,col=Affy_U133A_datasets.colour[[2]], las = 2) # Generates boxplot of non-normalized intensity values.
boxplot(data.frame(Affy_U133A_data),col=Affy_U133A_datasets.colour[[2]], main="Normalised GC-RMA data", las = 2) # Generates boxplot of normalized log intensity values.
dev.off()
...
Affy_U133A_data <- cbind(Affy_U133A_probes[intersect(rownames(Affy_U133A_data), rownames(Affy_U133A_probes)),1], Affy_U133A_data)

Affy_U133A_data  <- multiGene2ProbeFilter(Affy_U133A_data)

dim(Affy_U133A_data)

cat("Writing normalised expression data to Affy_U133A_gcrma.exp\n")
write.table(prepare2write(Affy_U133A_data), file="Affy_U133A_gcrma.exp",sep="\t", row.names=FALSE)
