...
gse_eset <- getGEO(GEO,GSEMatrix=TRUE)
print(gse_eset)
...
if(length(gse_eset) == 1) {
	...
	write.table(phenodata,file="target.txt",sep="\t",row.names=FALSE,quote=FALSE)
	write.table(exprsData,"normalised.txt",sep="\t", quote=FALSE, row.names=TRUE,col.names=NA)
