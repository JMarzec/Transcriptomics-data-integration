################################################################################
#
#   File name: GenExpressionComb_Affy_U95Av2.R
#
...
################################################################################
#
#	Description: Pipeline combining datasets produced using Affymetrix Human Genome U95Av2 Array. The script reads in three a priori generated files: (1) list of datasets to be analysed; (2) file produced with QC_Affy_U95Av2.R which lists outliers identified with ArrayOutliers function and (3) '.flat' file produced with ArrayAnnot.pl which lists reliable probes annotated with Ensembl gene IDs and gene symbol. Pipeline executes 'multiGene2ProbeFilter.R' to remove multi gene to probe mappings
#
#	Command line use example: R --file=./GenExpressionComb_Affy_U95Av2.R --args "/scratch/jack/data/PhD/Transcriptomics_project"
#
...
ProjectDir = args[4]
probesFlatFile = paste(paste(ProjectDir, collapse = "/"),"Affy_U95Av2.flat",sep="/")
...
DatasetInput=DatasetInput[DatasetInput[,"Platform"] %in% "Affy_U95Av2",]
...
cat("Loading datasets produced using Affymetrix GeneChip Human Genome U95Av2 Array into an AffyBatch object...\n")
...
Affy_U95Av2_probes <- read.table(probesFlatFile,sep="\t",as.is=TRUE,header=TRUE,row.names=1)
...
difference=setdiff(ls(hgu95av2cdf),rownames(Affy_U95Av2_probes))
##### Remove these genes from the cdf environment -> the cdf will have only the interesting genes
rm(list=difference, envir= hgu95av2cdf)
...
Affy_U95Av2_gcrma <- NULL
Affy_U95Av2_gcrma <- gcrma(dat)

#save.image("Affy_U95Av2_gcrma.R")
#load("Affy_U95Av2_gcrma.R")

Affy_U95Av2_data <- exprs(Affy_U95Av2_gcrma)

dim(Affy_U95Av2_data)

#===============================================================================
#    Boxplot on normalised data
#===============================================================================

Affy_U95Av2_datasets <- targetFile$Dataset

Affy_U95Av2_datasets_No <- max(as.numeric(factor(Affy_U95Av2_datasets)))

Affy_U95Av2_datasets.colour <- getDatasetsColours(Affy_U95Av2_datasets)


pdf("Affy_U95Av2_boxplot.pdf", pointsize = 8 ,width = 0.2*length(targetFile$FileName), height = 6)
par(mar=c(13, 4, 3, 2))
boxplot(dat,col=Affy_U95Av2_datasets.colour[[2]], las = 2) # Generates boxplot of non-normalized intensity values.
boxplot(data.frame(Affy_U95Av2_data),col=Affy_U95Av2_datasets.colour[[2]], main="Normalised GC-RMA data", las = 2) # Generates boxplot of normalized log intensity values.
dev.off()
...
Affy_U95Av2_data <- cbind(Affy_U95Av2_probes[intersect(rownames(Affy_U95Av2_data), rownames(Affy_U95Av2_probes)),1], Affy_U95Av2_data)

Affy_U95Av2_data  <- multiGene2ProbeFilter(Affy_U95Av2_data)

dim(Affy_U95Av2_data)

cat("Writing normalised expression data to Affy_U95Av2_gcrma.exp\n")
write.table(prepare2write(Affy_U95Av2_data), file="Affy_U95Av2_gcrma.exp",sep="\t", row.names=FALSE)
