################################################################################
#
#   File name: GenExpressionComb_Affy_U95B.R
#
...
################################################################################
#
#	Description: Pipeline combining datasets produced using Affymetrix Human Genome U95B Array. The script reads in three a priori generated files: (1) list of datasets to be analysed; (2) file produced with QC_Affy_U95B.R which lists outliers identified with ArrayOutliers function and (3) '.flat' file produced with ArrayAnnot.pl which lists reliable probes annotated with Ensembl gene IDs and gene symbol. Pipeline executes 'multiGene2ProbeFilter.R' to remove multi gene to probe mappings
#
#	Command line use example: R --file=./GenExpressionComb_Affy_U95B.R --args "/scratch/jack/data/PhD/Transcriptomics_project"
#
...
ProjectDir = args[4]
probesFlatFile = paste(paste(ProjectDir, collapse = "/"),"Affy_U95B.flat",sep="/")
...
DatasetInput=DatasetInput[DatasetInput[,"Platform"] %in% "Affy_U95B",]
...
cat("Loading datasets produced using Affymetrix GeneChip Human Genome U95B Array into an AffyBatch object...\n")
...
Affy_U95B_probes <- read.table(probesFlatFile,sep="\t",as.is=TRUE,header=TRUE,row.names=1)
...
difference=setdiff(ls(hgu95bcdf),rownames(Affy_U95B_probes))
##### Remove these genes from the cdf environment -> the cdf will have only the interesting genes
rm(list=difference, envir= hgu95bcdf)
...
Affy_U95B_gcrma <- NULL
Affy_U95B_gcrma <- gcrma(dat)

#save.image("Affy_U95B_gcrma.R")
#load("Affy_U95B_gcrma.R")

Affy_U95B_data <- exprs(Affy_U95B_gcrma)
...
Affy_U95B_datasets <- targetFile$Dataset

Affy_U95B_datasets_No <- max(as.numeric(factor(Affy_U95B_datasets)))

Affy_U95B_datasets.colour <- getDatasetsColours(Affy_U95B_datasets)


pdf("Affy_U95B_boxplot.pdf", pointsize = 8 ,width = 0.2*length(targetFile$FileName), height = 6)
par(mar=c(13, 4, 3, 2))
boxplot(dat,col=Affy_U95B_datasets.colour[[2]], las = 2) # Generates boxplot of non-normalized intensity values.
boxplot(data.frame(Affy_U95B_data),col=Affy_U95B_datasets.colour[[2]], main="Normalised GC-RMA data", las = 2) # Generates boxplot of normalized log intensity values.
dev.off()
...
Affy_U95B_data <- cbind(Affy_U95B_probes[intersect(rownames(Affy_U95B_data), rownames(Affy_U95B_probes)),1], Affy_U95B_data)

Affy_U95B_data  <- multiGene2ProbeFilter(Affy_U95B_data)

dim(Affy_U95B_data)

cat("Writing normalised expression data to Affy_U95B_gcrma.exp\n")
write.table(prepare2write(Affy_U95B_data), file="Affy_U95B_gcrma.exp",sep="\t", row.names=FALSE)
