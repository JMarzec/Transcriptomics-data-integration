################################################################################
#
#   File name: DiffExpression_openPlatform.R
#
...
#
#   Command line use example: R --file=./DiffExpression_openPlatform.R --args "/scratch/jack/data/PhD/Transcriptomics_project" "E-MTAB-567,TCGA" "0.000000001" "1" "BH" "nonparam" "/scratch/jack/data/PhD/Transcriptomics_project/stromal_genes_all.txt"
#
#   First arg:      Project workspace. This is the directory to which cross-platform analysis results will be written
#   Second arg:     Datasets, processed with seleceted platform in third arg to be considered for the analysis. Each datasets ID is expected to be separated by comma. Type 'all' for all listed datasets to be analysed
#   Third arg:      Adjustep p-value to be used as a threshold for detecting differentially expressed genes
#   Fourth arg:     Absolute value of logarithmic fold change to be used as a threshold for detecting differentially expressed genes
#   Fifth arg:      P-value adjustment method. Possible values are "none", "BH", "fdr" (equivalent to "BH"), "BY" and "holm"
#   Sixth arg (OPTIONAL): If "nonparam", use nonparametric adjustments in ComBat. AS default, parametric adjustments are used
#   Seventh arg (OPTIONAL): List of genes to be excluded from the differential expression analysis. The first column is expected to list the Ensembl gene IDs
#
################################################################################
...
} else if (!is.na(args[9]) && args[9] != "nonparam") {
...
if ( datasets2corr != "all" && datasets2corr != "ALL" ) {
    ...
    if ( all(datasets2corr %in% rownames(DatasetInput)) ) {
        ...
        cat(paste("\nSome of selected datasets (", paste(datasets2corr, collapse = ", "), ") were not generated by RNA-seq!\n\n", sep=" "))
        q()
    }
} else {
...
datasetList <- NULL
datasetList[[1]] <- paste(ProjectDir,"RNAseq_cqn.exp",sep="/")
...
##### Use 50% of all genes with the highest expression variance across samples in the non-specific filtering step
filterThreshold <- round(0.5*nrow(data_combat), digits = 0)

##### Keep 50% of all genes with the highest expression variance across samples
...
data_combat[data_combat == 1] <- 0

##### Form a DGEList object
DGEdata <- DGEList(counts=data_combat, genes=rownames(data_combat))
DGEdata$samples$lib.size <- colSums(DGEdata$counts)

##### Apply scale normalisation
#DGEdata <- calcNormFactors(DGEdata)

cat("Differenatial expression analysis using limma...\n")
...
pdf(file = paste("Comb_", paste(datasets2corr, collapse="_"), "_voomPlot", ".pdf", sep = ""))
DGEdata.log2 <- voom(DGEdata, mod, plot=TRUE)
dev.off()
...
if ( all(is.na(targetFile$Replicate)) ) {
    ...
    corfit <- duplicateCorrelation(DGEdata.log2, design=mod, ndups = 1, block = biolrep)
    fit <- lmFit(DGEdata.log2, mod, block = biolrep, cor = corfit$consensus)
}
...
for (i in 1:ncol(eb$p.value) ) {
    ...
    topGenes <- topTable(eb, coef=colnames(eb)[i], adjust=adjMethod, sort.by="none", number=nrow(DGEdata.log2))
    ...
    top10 <- ord[1:10]
    text(topGenes[top10,"logFC"],-log2(topGenes[top10,"adj.P.Val"]), labels=rownames(DGEdata.log2[top10,]),cex=0.6,col="blue")
    dev.off()
}
...
for (i in 1:ncol(eb$p.value) ) {
	...
	topGenes <- topTable(eb, coef=colnames(eb)[i], adjust=adjMethod,  sort.by="p", number=nrow(DGEdata.log2))
    rownames(topGenes) <- topGenes[,"genes"]
...
for (i in 1:ncol(eb$p.value) ) {
	...
	topGenes <- topTable(eb, coef=colnames(eb)[i], adjust=adjMethod,  sort.by="p", p.value=pThreshold, lfc=lfcThreshold, number=nrow(DGEdata.log2))
...
topGenes <- DGEdata.log2$E[unique(topGenes.index),]
dim(topGenes)
...
if (nrow(topGenes) > 0) {
    ...
    par(mar=c(2,5,2,0))
    plot(hc, xlab="", labels=paste(colnames(DGEdata.log2), targets, sep="   "), hang = -1, main="")
    dev.off()
