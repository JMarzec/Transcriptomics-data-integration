...
ProjectDir = args[4]
probesFlatFile = paste(paste(ProjectDir, collapse = "/"),"Affy_U133Plus2.flat",sep="/")
...
Affy_U133Plus2_probes <- read.table(probesFlatFile,sep="\t",as.is=TRUE,header=TRUE,row.names=1)
...
difference=setdiff(ls(hgu133plus2cdf),rownames(Affy_U133Plus2_probes))
##### Remove these genes from the cdf environment -> the cdf will have only the interesting genes
rm(list=difference, envir=hgu133plus2cdf)
