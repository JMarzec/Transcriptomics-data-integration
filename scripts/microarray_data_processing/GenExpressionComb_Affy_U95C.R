################################################################################
#
#   File name: GenExpressionComb_Affy_U95C.R
#
...
################################################################################
#
#	Description: Pipeline combining datasets produced using Affymetrix Human Genome U95C Array. The script reads in three a priori generated files: (1) list of datasets to be analysed; (2) file produced with QC_Affy_U95C.R which lists outliers identified with ArrayOutliers function and (3) '.flat' file produced with ArrayAnnot.pl which lists reliable probes annotated with Ensembl gene IDs and gene symbol. Pipeline executes 'multiGene2ProbeFilter.R' to remove multi gene to probe mappings
#
#	Command line use example: R --file=./GenExpressionComb_Affy_U95C.R --args "/scratch/jack/data/PhD/Transcriptomics_project"
#
...
ProjectDir = args[4]
probesFlatFile = paste(paste(ProjectDir, collapse = "/"),"Affy_U95C.flat",sep="/")
...
DatasetInput=DatasetInput[DatasetInput[,"Platform"] %in% "Affy_U95C",]
...
cat("Loading datasets produced using Affymetrix GeneChip Human Genome U95C Array into an AffyBatch object...\n")
...
Affy_U95C_probes <- read.table(probesFlatFile,sep="\t",as.is=TRUE,header=TRUE,row.names=1)
...
difference=setdiff(ls(hgu95ccdf),rownames(Affy_U95C_probes))
##### Remove these genes from the cdf environment -> the cdf will have only the interesting genes
rm(list=difference, envir= hgu95ccdf)
...
Affy_U95C_gcrma <- NULL
Affy_U95C_gcrma <- gcrma(dat)

#save.image("Affy_U95C_gcrma.R")
#load("Affy_U95C_gcrma.R")

Affy_U95C_data <- exprs(Affy_U95C_gcrma)
...
Affy_U95C_datasets <- targetFile$Dataset

Affy_U95C_datasets_No <- max(as.numeric(factor(Affy_U95C_datasets)))

Affy_U95C_datasets.colour <- getDatasetsColours(Affy_U95C_datasets)


pdf("Affy_U95C_boxplot.pdf", pointsize = 8 ,width = 0.2*length(targetFile$FileName), height = 6)
par(mar=c(13, 4, 3, 2))
boxplot(dat,col=Affy_U95C_datasets.colour[[2]], las = 2) # Generates boxplot of non-normalized intensity values.
boxplot(data.frame(Affy_U95C_data),col=Affy_U95C_datasets.colour[[2]], main="Normalised GC-RMA data", las = 2) # Generates boxplot of normalized log intensity values.
dev.off()
...
Affy_U95C_data <- cbind(Affy_U95C_probes[intersect(rownames(Affy_U95C_data), rownames(Affy_U95C_probes)),1], Affy_U95C_data)

Affy_U95C_data  <- multiGene2ProbeFilter(Affy_U95C_data)

dim(Affy_U95C_data)

cat("Writing normalised expression data to Affy_U95C_gcrma.exp\n")
write.table(prepare2write(Affy_U95C_data), file="Affy_U95C_gcrma.exp",sep="\t", row.names=FALSE)
